          ...
          <el-table :data="manyTableData" border stripe>
            ...
            <el-table-column type="expand">
              <template slot-scope="scope">
                <!--循环渲染Tag标签-->
                <el-tag class="el-tag" v-for="(item, i) in scope.row.attr_vals" :key="i" closable @close="handleClose(i, scope.row)">{{ item }}</el-tag>
                <!--输入的文本框-->
                <el-input
                  class="input-new-tag"
  ...
  methods: {
    ...
    editParams () {
      this.$refs.editRef.validate(async valid => {
        ...
        if (res.meta.status !== 200) return this.$message.error('修改参数名称失败')
    ...
    async removeParams (attrId) {
      ...
      if (result !== 'confirm') return this.$message.info('已取消删除操做')
